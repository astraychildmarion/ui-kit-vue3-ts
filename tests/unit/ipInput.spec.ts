...
test('enter large number, and return 255', async () => {
  ...
  const input1 = component.get('.ant-input');
...
test('enter all input with right number', async () => {
  const component = mount(IpInput);
  const input1 = component.get('input[name="value1"]');
  const input2 = component.get('input[name="value2"]');
  const input3 = component.get('input[name="value3"]');
  const input4 = component.get('input[name="value4"]');
  await input1.setValue('123');
  await input2.setValue('123');
  await input3.setValue('123');
  await input4.setValue('0');
  expect(component.emitted('changeIp')[6][0]).toEqual('123.123.123.0');
});
test('enter all input with 2 wrong number', async () => {
  ...
  expect(component.emitted('changeIp')[6][0]).toEqual('123.123.255.255');
});
